...
// OFF-023 adapter-schedule — package boundary self-gate. These checks mirror
// the work item's acceptance boundary: @office/adapter-schedule declares
...
// (comments may describe the Primavera-CLASS family generically; identifiers
// and string values stay generic — 'schedule-pm' over
// 'schedule-instance-01'), and performs NO I/O of its own (ports only: no
// fs, no net, no process spawning, no database env, no SQL, no migrations —
// and no wall clock or randomness anywhere: injected clock/id suppliers
// only). Deterministic: filesystem reads only.
...
// test's own source.
const ownPackageName = ['@office', '/adapter-schedule'].join('');
...
// reference it (the frozen OFF-023 boundary).
...
// match its own source. Comments may describe the Primavera-CLASS adapter
...
describe('adapter-schedule package boundary (OFF-023)', () => {
  it('carries the canonical adapter-schedule package identity (0.1.0, private, ESM)', () => {
  ...
  it('contains no provider vocabulary in any source or test file CODE (comments excluded)', () => {
    const violations: string[] = [];
    for (const file of listFiles(srcDir, '.ts')) {
      // Comments may carry the work item's own 'Primavera-class' descriptor;
  ...
  it('keeps the generic schedule vocabulary discipline (no vendor identity in values)', () => {
    ...
    expect(vocabulary).toContain("'schedule-pm'");
    expect(vocabulary).toContain("'schedule-instance-01'");
    expect(vocabulary).toContain("'project-schedule'");
    expect(vocabulary).toContain("'activity'");
    expect(vocabulary).toContain("'activity-dependency'");
    expect(vocabulary).toContain("'baseline'");
    // The fixture's object ids stay generic provider-side coordinates.
    const fixture = readFileSync(join(srcDir, 'provider-fixture.ts'), 'utf8');
    expect(fixture).toContain("'sch-tower-a'");
    expect(fixture).toContain("'act-401'");
    expect(fixture).toContain("'dep-401-402'");
    expect(fixture).toContain("'bl-2026-09'");
  });
