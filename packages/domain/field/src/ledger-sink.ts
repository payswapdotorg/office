// Office field domain — the ledger-backed EventSink adapter (OFF-009).
//
...
// the envelope's entityRefs.after (this domain's events always carry the
...
// in-memory-testable (see the integration suite for the real-PostgreSQL
// proof of the port).
  ...
  invariantViolation(
    {
      ...
      statement: `event '${eventName}' carries no entityRefs.after reference; the ledger-backed sink derives each append's aggregate from it (a field event must address its mutated aggregate)`,
    },
  );
