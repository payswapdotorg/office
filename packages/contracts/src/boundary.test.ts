...
// OFF-002 contracts — package boundary self-gate. These checks mirror the
// work item's acceptance boundary: @office/contracts is the innermost shared
// kernel (zero dependencies, no imports outside the package) and carries no
// provider vocabulary. Deterministic: filesystem reads only.
...
describe('contracts package boundary (OFF-002)', () => {
  it('is @office/contracts 0.1.0, private, ESM', () => {
    const pkg = readPackageJson();
    expect(pkg['name']).toBe('@office/contracts');
  ...
  it('imports nothing outside the package (innermost kernel)', () => {
