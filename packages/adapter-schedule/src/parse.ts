// Office adapter-schedule — package-internal parse plumbing (OFF-023).
//
...
// parseFail — the same typed ContractParseError surface every Office
// boundary uses). Mirrors the internal helper layer of packages/adapter-model
// (OFF-022), which itself mirrors packages/adapters-sdk (OFF-020): those
// helpers are deliberately package-internal THERE, so this module repeats the
// small combinator set for the schedule adapter, plus the bounded-integer and
// boolean combinators the CPM payload contracts need (durations, lag days,
// the baseline protection flag).
//
...
/** Require a nullable string field (null accepted; otherwise the rule runs). */
export const requireNullableString = (
  raw: Record<string, unknown>,
  field: string,
  path: string,
  rule: StringRule,
...
/** Require a boolean field of an object (missing/type are typed failures). */
export const requireBoolean = (
  ...
  const value = raw[field];
  if (value === undefined) {
    return parseFail('missing-field', fieldPath(path, field), description, 'undefined');
  }
  if (typeof value !== 'boolean') {
    return parseFail('invalid-type', fieldPath(path, field), description, describeValue(value));
  }
  return parseOk(value);
};
...
/** Require a positive finite number field (aggregate versions, counts). */
export const requirePositiveNumber = (
  ...
  if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) {
    return parseFail('invalid-value', fieldPath(path, field), description, describeValue(value));
  }
  return parseOk(value);
};

/** Require an integer field within inclusive bounds (durations, lag days). */
