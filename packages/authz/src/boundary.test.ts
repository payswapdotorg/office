...
// OFF-006 authz — package boundary self-gate. These checks mirror the work
// item's acceptance boundary: @office/authz depends ONLY on
// @office/contracts and @office/domain-kernel (dependency rule: domain
// modules depend only inward on the shared kernel contracts), imports
// nothing else outside the package, and carries no provider vocabulary.
...
describe('authz package boundary (OFF-006)', () => {
  it('is @office/authz 0.1.0, private, ESM', () => {
    const pkg = readPackageJson();
    expect(pkg['name']).toBe('@office/authz');
    expect(pkg['version']).toBe('0.1.0');
    expect(pkg['private']).toBe(true);
    expect(pkg['type']).toBe('module');
  });

  it('declares exactly two runtime dependencies: @office/contracts and @office/domain-kernel (workspace)', () => {
    const pkg = readPackageJson();
  ...
  it('imports only @office/contracts and @office/domain-kernel outside the package (plus test-only vitest/node builtins)', () => {
  ...
  it('keeps the required policy and scope modules present', () => {
    for (const required of ['policy.ts', 'scope.ts']) {
