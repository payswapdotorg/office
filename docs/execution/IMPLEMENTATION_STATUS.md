...
- Production declaration (Stages 3–7, the completion of issue #55): the hosted Office app is LIVE at `office-teal-zeta.vercel.app` over real production PostgreSQL — Neon project `office-production` (`square-paper-16539172`, `aws-us-east-2`, PostgreSQL 17; the app on the pooled endpoint, the migration operator on the direct one). The bootstrap operator procedure applied the ordered migration union 6/6 and seeded the canonical chain (tenant → organization → project) through the REAL command paths; the first database-backed production deployment `dpl_CEZA2TFDs11Wd1ghLe6hResgohYf` (commit `e380b01`) answers `/api/health` 200 reachable + 6 applied + release identity; the hosted smoke acceptance chain passed 23/23 against the live URL (readiness, every read surface, the typed 400 discipline, the field-capture canonical command, the A8 approval-gated submit → propose → complete with `workflows.approvalApproved` in the transactional ledger); the client-bundle scan found zero secret material; `DATABASE_URL` lives as an encrypted Vercel environment variable (server-only, production + preview); the rollback procedure is machine-verified (the alias-assignment API against the live project) with the rollback target `dpl_Greqbz4Tyc3mgPz6agBQ2xFK4mF1` (READY). Every hosting fact, environment class, and operator procedure is recorded in docs/execution/DEPLOYMENT.md (the runbook).
