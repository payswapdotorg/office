// Office field domain — package-internal parse helpers (OFF-009).
//
// Fail-closed payload parsing for the domain's command payloads and read-model
// inputs, mirroring the conventions of @office/contracts' internal parse
// plumbing and the sibling identity packages (total parsers returning
...
//
// Additions over the identity packages' helper set (the field model needs
// them): closed literal-vocabulary parsing, finite-number parsing for
// quantities/measurements, and canonical calendar-day parsing for daily logs.
 ...
 * that already include it (the list parsers below report 'evidence',
 * 'evidence[1]', ...): the missing-field probe mirrors requireFieldWith,
 * but a present value is NOT re-nested — the sub-parser's dotted paths are
 * already absolute, so composition must not prefix the field twice.
 */
...
/**
 * Parse a finite number value (quantities/measurements): must be a JSON
 * number, finite (never NaN/Infinity — JSON cannot carry them and a
 * measurement must be exact).
 */
export const parseFiniteNumber = (raw: unknown): ParseResult<number> => {
  if (typeof raw !== 'number') {
    return parseFail('invalid-type', '', 'a finite number', describeValue(raw));
  }
  if (!Number.isFinite(raw)) {
    return parseFail('invalid-value', '', 'a finite number', describeValue(raw));
  }
  return parseOk(raw);
};

/**
 * Parse a closed literal vocabulary (statuses, severities, checklist
 * results): fail-closed against the exact allowed literals.
 */
 ...
 * Duplicates are rejected when allowed is given as a description of identity.
 */
export const parseValueArrayWith = <T>(
  ...
  for (const [index, element] of raw.entries()) {
    ...
    if (!result.ok) {
      ...
      // separator dot between the field and the bracket (e.g. 'items[2]').
      return parseFail(
        result.error.code,
...
/** Canonical calendar day string (daily-log scoping): 'YYYY-MM-DD', UTC. */
export type LogDay = string & { readonly __logDay: 'LogDay' };

/** Grammar description used in parse failures. */
export const LOG_DAY_GRAMMAR =
  'YYYY-MM-DD — canonical calendar day (UTC), month/day calendar-exact';

const LOG_DAY_PATTERN = /^(\d{4})-(\d{2})-(\d{2})$/;

/**
 * Parse an untrusted value as a LogDay (total, fail-closed, calendar-exact —
 * including leap years, mirroring the contracts timestamp conventions).
 */
export function parseLogDay(raw: unknown): ParseResult<LogDay> {
  if (typeof raw !== 'string') {
    return parseFail('invalid-type', '', LOG_DAY_GRAMMAR, describeValue(raw));
  }
  const match = LOG_DAY_PATTERN.exec(raw);
  if (match === null) {
    return parseFail('invalid-value', '', LOG_DAY_GRAMMAR, describeValue(raw));
  }
  ...
  if (!calendarValid) {
    return parseFail('invalid-value', '', 'a valid calendar day', describeValue(raw));
  }
  return parseOk(raw as LogDay);
}

/** Type guard for structurally valid LogDay values. */
export const isLogDay = (raw: unknown): raw is LogDay => parseLogDay(raw).ok;

/**
 * Parse a standalone JSON object value: must be a plain JSON object — never
 * null, never an array, never a scalar.
 */
export const parseJsonObject = (
  ...
  isPlainObject(raw)
    ? parseOk(raw)
    : parseFail('invalid-type', '', 'a JSON object', describeValue(raw));
