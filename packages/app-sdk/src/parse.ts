// Office app-sdk — package-internal parse plumbing (OFF-025).
//
// Total, fail-closed parse combinators for the app-sdk package's own value
// types, built on the PUBLIC contracts parse plumbing (ParseResult, parseOk,
// parseFail — the same typed ContractParseError surface every Office
// boundary uses). Mirrors the internal helper layers of the contracts,
// authz, and adapters-sdk packages: those helpers are deliberately
// package-internal THERE, so this module repeats the small combinator set
// for the app SDK.
//
 ...
 * nested element failure reads `permissions[0].capability`, never
 ...
 * OWN internal path when it has one ('<field>[3].capability'). Duplicates
 * are NOT rejected here (value identity cannot see structural duplicates) —
 * set-like semantics are the caller's rule, keyed by its own logical key.
 */
