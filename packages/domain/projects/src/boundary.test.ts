...
// OFF-007 project domain — package boundary self-gate. These checks mirror
// the work item's acceptance boundary (and the sibling organization module's
// self-gate): @office/domain-projects declares exactly four runtime
// dependencies (@office/contracts, @office/domain-kernel, @office/authz,
// @office/persistence — the declared dependency graph of OFF-007), imports
// nothing else outside the package outside of tests, NEVER imports
// @office/events (out of this item's dependency graph — the EventSink port
// is the seam the ledger implements), carries no provider vocabulary, and
// ships its forward-only migration co-located with the package (version 0101
// onward, applied through @office/persistence's migrator conventions).
...
describe('project domain package boundary (OFF-007)', () => {
  it('is @office/domain-projects 0.1.0, private, ESM', () => {
    const pkg = readPackageJson();
    expect(pkg['name']).toBe('@office/domain-projects');
  ...
  it('ships its co-located migration numbered 0101 onward (forward-only, persistence migrator conventions)', () => {
    ...
    for (const version of versions) {
      expect(version).toBeGreaterThanOrEqual(101);
    }
