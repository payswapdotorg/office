...
// OFF-012 contracts/change domain — package boundary self-gate. These checks
// mirror the work item's acceptance boundary: @office/domain-contracts
// declares exactly five workspace dependencies (@office/contracts,
// @office/domain-kernel, @office/authz, @office/events — IN this item's
// dependency graph for the thin ledger-backed EventSink adapter — and
// @office/persistence for the SqlExecutor type of the mirrored EventSink
// port), imports nothing else outside the package outside of tests, NEVER
// imports another domain package (no domain-to-domain imports —
// cross-entity references are typed EntityId/EntityRef links), carries no
// provider vocabulary, and ships NO persistence of its own (pure domain: no
// migrations, no SQL — the in-memory store is the transactional seam tests
// use). Deterministic: filesystem reads only.
...
// below). No domain-to-domain imports: the contracts/change package consumes
// shared kernel contracts only, never a sibling domain package (and never
// itself by package name — the surface is src/index.ts).
...
const ownPackageName = ['@office', '/domain-contracts'].join('');
...
// this package — the contracts/change contract is Office-canonical;
// provider import happens in adapter packages owned elsewhere).
...
describe('contracts/change domain package boundary (OFF-012)', () => {
  it('carries the canonical contracts package identity (0.1.0, private, ESM)', () => {
  ...
  it('is deterministic: no wall clock or randomness in the source modules', () => {
    const violations: string[] = [];
    for (const file of listFiles(srcDir, '.ts')) {
      if (file.endsWith('.test.ts')) continue;
      const text = stripComments(readFileSync(join(srcDir, file), 'utf8'));
      if (text.includes('Date.now')) violations.push(`${file}: Date.now`);
      if (text.includes('Math.random')) violations.push(`${file}: Math.random`);
    }
    expect(violations).toStrictEqual([]);
  });
});
