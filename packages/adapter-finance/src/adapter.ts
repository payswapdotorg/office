// Office adapter-finance — the Adapter implementation (OFF-024).
//
// A complete Adapter over the @office/adapters-sdk contract, driven entirely
// by an injected ERP provider store (provider-fixture.ts — swapped for a
// real ERP client by the runtime, never by the translation seams):
//
//   - CAPABILITIES: the five declared object-kind surfaces (account →
//     canonical 'budget', cost-code → 'cost-item', commitment →
//     'commitment', invoice → 'invoice', payment → 'payment-reference'),
//     each with the authz capability the engines authorize through
//     (deny-by-default);
...
//     second, idempotent layers are the version ledger and the SDK's
...
import { FINANCE_ADAPTER_KIND, FINANCE_CAPABILITIES, FINANCE_SYSTEM_ID } from './vocabulary';
...
export interface FinanceAdapterParts {
  /** The provider store the adapter reads (the fixture, or a real ERP client's read surface). */
  readonly store: ErpProviderStore;
  /** The provider system this adapter serves (defaults to the fixture's). */
  readonly systemId?: ProviderSystemId;
}

/** Create the ERP/finance Adapter over one injected provider store. */
export function createFinanceAdapter(parts: FinanceAdapterParts): Adapter {
  const systemId = parts.systemId ?? FINANCE_SYSTEM_ID;
  ...
  const unknownConnection = (): DomainError =>
    domainError(
      'not-found',
      `no finance adapter connection to system '${systemId}' matches the presented connection — connect first`,
      [
        {
  ...
  return {
    kind: FINANCE_ADAPTER_KIND,
    capabilities: FINANCE_CAPABILITIES satisfies AdapterCapabilities,
    ...
    async connect(request) {
      if (request.systemId !== systemId) {
        return fail(
          domainError(
            'invariant-violation',
            `ERP provider serves system '${systemId}', not '${request.systemId}'`,
            [
              {
    ...
    async sync(request: SyncRequest): Promise<Result<SyncResult, DomainError>> {
      ...
      if (!Number.isInteger(start) || start < 0 || start > stream.length) {
        return fail(
          domainError(
            'invariant-violation',
            `ERP provider cannot resume object kind '${request.objectKind}' from token '${request.cursor?.token ?? '<null>'}'`,
            [
              {
