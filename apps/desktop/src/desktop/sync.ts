// Office desktop client protocol/reference shell — the reconnect +
// synchronize surface (OFF-032).
//
// THE synchronization surface of the desktop shell: the reconnect flow
// drives @office/client-sync's engine.reconnect — catchup (resume from the
// client's LAST CONFIRMED cursor, exactly once, no duplicates, no gaps) +
// the queue's EXACTLY-ONCE drain (authorize → register → divergence check →
// clean-apply or the explicit conflict path — conflict surfacing included) +
// the consumption of the client's own replayed effects. The engine's
// ReconnectReport becomes the displayable SYNC REPORT VIEW
// (applied/rejected/conflicted/superseded counts, per-entry outcomes, the
// catchup window, the remaining pending count) and the client's cursor/token
// state is displayable at every step (the consumed position IS the
// causal-token basis of the next capture).
//
// THE SAME client-sync engine the field shell synchronizes through — the
// desktop goes offline too. This module is a pure projection over the
// engine's typed reports: no I/O, no clock reads (the `now` of the
// reconnect is injected), no state of its own. Typed Results throughout: a
// session-state violation (synchronizing a connected session, say) is a
...
import type { ConsumedEventView } from './workspace';
 ...
 * before the report returns, so every applied effect the client keeps is in
 * the lookup — the applied view never invents a name).
 */
