// Office project domain — package-internal parse helpers (OFF-007).
//
...
// received-value descriptions) and byte-for-byte the helpers of the sibling
// @office/domain-organization package (the two domain modules stay
// independent — no domain-to-domain imports — so each carries its own copy).
// NOT exported from the package root: only the command modules of this
...
/**
 * Parse a JSON object field (extension metadata, freeze A2): must be a plain
 * JSON object — never null, never an array, never a scalar.
 */
export const requireJsonObject = (
  raw: Record<string, unknown>,
  field: string,
  path: string,
): ParseResult<Readonly<Record<string, unknown>>> => {
  ...
  if (value === undefined) {
    return parseFail(
      ...
      'a JSON object (extension metadata)',
      'undefined',
    );
  }
