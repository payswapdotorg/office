// Office intelligence — package-internal test support (OFF-014).
//
// NOT part of the public surface: deterministic factories for the command
// and event envelopes the golden scenario streams need, mirroring the
// landed @office/intelligence-relationships test-support idioms exactly
...
// ---------------------------------------------------------------------------
// Assessment-authorization factories (deterministic, deny-by-default probes).
// ---------------------------------------------------------------------------

/** Every area read capability an assessment requires. */
...
/**
 * An assessment authorization for the fixed test user: the given scope, the
 ...
 * capability-missing probes of the assessment tests.
 */
export const assessmentAuthorizationOf = (
  scope: Scope,
  parts: {
  ...
  context: authorizationContext({
    actor: USER_ACTOR,
    scope,
    capabilities: parts.capabilities ?? ALL_ASSESSMENT_CAPABILITIES,
  }),
});

/** The project-one reader every golden scenario assesses under. */
export const projectOneReader = (): AssessmentAuthorization =>
