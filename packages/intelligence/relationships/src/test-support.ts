// Office intelligence — package-internal test support (OFF-013).
//
// NOT part of the public surface: deterministic factories for the command
// and event envelopes the golden streams need. The domain packages cannot
// be imported (the relationship engine is a projection, not a domain peer),
// so their envelope-builder behavior is reproduced here through the
// canonical contracts: commands parse through parseCommandEnvelope, events
// self-check through parseDomainEventEnvelope, and event causality is
// derived through the REAL @office/events conventions (causedByCommand /
// causedByEvent) — exactly the shapes the landed packages emit. Fixed
...
export const projectScopeOf = (projectId: ProjectId): Scope => ({
  kind: 'project',
  tenantId: TENANT_A,
  projectId,
});
...
export const testId = (prefix: string, n: number): ReturnType<typeof formatEntityId> =>
...
// ---------------------------------------------------------------------------
// Traversal-authorization factories (deterministic, deny-by-default probes).
// ---------------------------------------------------------------------------

/**
 * Every area read capability the engine's entity-kind vocabulary gates on
 * (the seven bounded-context areas its kinds belong to).
 */
export const ALL_AREA_READ_CAPABILITIES: readonly string[] = [
  'organization.read',
  'projects.read',
  'documents.read',
  'work.read',
...
/**
 * A traversal authorization for the fixed test user: the given scope, the
 * given capabilities (default: every area read), and the given policy
 * (default: allow-all-reads). Capabilities/policy combos are the A12 and
 * capability-missing probes of the traversal tests.
 */
export const traversalAuthorizationOf = (
  scope: Scope,
  parts: {
  ...
  context: authorizationContext({
    actor: USER_ACTOR,
    scope,
    capabilities: parts.capabilities ?? ALL_AREA_READ_CAPABILITIES,
  }),
});
