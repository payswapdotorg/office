...
import { ACTIVITY_ADDED_EVENT } from './events';
import { ACTIVITY_KIND } from './state';
import { scheduleEventEnvelope } from './events';
import { createLedgerEventSink } from './ledger-sink';

// OFF-010 schedule domain — the thin ledger-backed EventSink adapter,
...
const SCHEDULE_ID = formatEntityId({
  version: 'v1',
...
const activityAddedEnvelope = (): DomainEventEnvelope =>
  scheduleEventEnvelope({
    ...
    entityRefs: {
      before: null,
      after: { entityKind: ACTIVITY_KIND, entityId: ACTIVITY_ID },
    },
    payload: {
...
describe('createLedgerEventSink (shape-level, fake executor)', () => {
  it('appends one event through the ledger and outbox inside the caller transaction', async () => {
    ...
    const result = await sink.appendEvents(executor, [activityAddedEnvelope()]);
    ...
    // The ledger insert is scoped to the SCHEDULE aggregate stream derived
    // from the event payload's owning scheduleId (binds $1..$4).
    const ledgerBinds = executor.values[1] ?? [];
    expect(ledgerBinds[0]).toBe(TENANT_A);
    expect(ledgerBinds[2]).toBe('schedule');
    expect(ledgerBinds[3]).toBe(SCHEDULE_ID);
    expect(ledgerBinds[5]).toBe('schedule.activityAdded');
  ...
  it('fails closed when an envelope payload does not carry the owning scheduleId', async () => {
    const executor = fakeExecutor();
    const sink = createLedgerEventSink();
    const bare = unwrap(
      parseDomainEventEnvelope({
        ...
        payload: { note: 'no owning schedule id' },
      }),
    );
    ...
    if (!result.ok) {
      ...
      expect(result.error.details[0]?.code).toBe('schedule-event-payload-carries-schedule-id');
    }
