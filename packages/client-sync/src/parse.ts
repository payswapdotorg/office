// Office client-sync — package-internal parse plumbing (OFF-029).
//
// Total, fail-closed parse combinators for the offline sync engine's own
// value types (local sequences, queue entries and their lifecycle states,
// protection classes), built on the PUBLIC contracts parse plumbing
// (ParseResult, parseOk, parseFail — the same typed ContractParseError
// surface every Office boundary uses). Mirrors the internal helper layers
// of the contracts, authz, and sync packages: those helpers are deliberately
// package-internal there, so this module repeats the small combinator set
// for client-sync (a documented structural mirror, exactly like the sync
// package's own parse layer mirrors contracts).
//
