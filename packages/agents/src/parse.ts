// Office agent runtime — package-internal parse helpers (OFF-018).
//
// Fail-closed parsing for the runtime's own value objects (run ids, tool
// names/descriptors, evidence queries/items/sets, proposed actions, audit
// payloads), mirroring the conventions of @office/contracts' internal parse
// plumbing and the landed packages (@office/actions parses its own shapes the
// same way — the helpers are mirrored, not imported, because they are
// package-internal surfaces): total parsers returning ParseResult, strict
// keys, dotted error paths, short received-value descriptions, closed literal
// vocabularies.
//
// NOT exported from the package root: only the modules of this package use
// these helpers; run/tool/evidence shapes are the package's own contract
...
/**
 * Parse a closed literal vocabulary (run statuses, tool kinds): fail-closed
 * against the exact allowed literals.
 */
