// Office cost domain — package-internal parse helpers (OFF-011).
//
// Fail-closed payload parsing for the domain's command payloads, mirroring
// the conventions of the sibling pure-domain modules (total parsers returning
// ParseResult, strict keys, dotted error paths, short received-value
// descriptions). NOT exported from the package root: only the command and
// state modules of this package use these helpers; payload shapes are the
// package's own contract surface and are re-exported as typed values.
//
// Cost-specific additions over the schedule helpers: bounded INTEGER rules
// for money amounts in integer minor units (amounts NEVER float — every
// stored figure is an exact integer, so balances are exact by construction),
// an exact-integer ARRAY parser for line lists (commitment/invoice lines),
// and the pure chronological timestamp comparison (issue/due-date sanity) —
// no host Date parsing, fully deterministic.
 ...
 * value delegates to the sub-parser. Used for optional descriptions, reasons
 * and invoice issue/due dates.
 */
...
/**
 * Require a field that holds an ARRAY of plain objects, with item-count
 * bounds, parsing each element through `parseItem` with error paths nested
 * under `field.<index>`. Fail-closed: a non-array, a non-object element, or
 * an out-of-bounds item count is a typed parse failure — never silently
 * coerced. Deterministic: elements are parsed in array order.
 */
export const requireObjectArray = <T>(
  raw: Record<string, unknown>,
  field: string,
  path: string,
  grammar: string,
  options: { readonly min: number; readonly max: number },
  parseItem: (item: Record<string, unknown>, itemPath: string) => ParseResult<T>,
): ParseResult<T[]> => {
  const value = raw[field];
  const at = fieldPath(path, field);
  if (value === undefined) {
    return parseFail('missing-field', at, grammar, 'undefined');
  }
  if (!Array.isArray(value)) {
    return parseFail('invalid-type', at, grammar, describeValue(value));
  }
  ...
  for (const [index, item] of value.entries()) {
    const itemPath = joinPath(at, String(index));
    if (!isPlainObject(item)) {
      return parseFail('invalid-type', itemPath, grammar, describeValue(item));
    }
