// Office field/offline web client — the session's offline data plane (OFF-031).
//
// The field session's data plane over @office/sync + @office/client-sync: the
// session joins ONE project slice's stream through an explicit A9
// subscription grant, and the plane wires @office/client-sync's SyncEngine
// over the SEEDED WORLD's SHARED server-side parts (the broker, the slice
// source, the operation registry, the applied-operation journal, the
// conflict log, the sync audit sink, and the world's typed command path) —
// two planes over one world is the field-client/office-twin convergence
// composition, exactly like the landed engine tests' two-client world.
//
...
//
// Mirrors the landed @office/web shell's data-plane discipline (the
...
import type { SeededFieldWorld } from './world';
...
/**
 * One field mutation the session submits through its data plane: the typed
...
export type FieldMutationRequestRejection =
  | { readonly code: 'invalid-operation-kind'; readonly received: string };

/** The field session's data plane (one client's sync session). */
export interface FieldDataPlane {
  readonly kind: 'field-data-plane';
   ...
   * office-side session's mutations). Typed Results, never throws.
   */
  submit(
    ...
    Result<
      ...
      DomainError | FieldMutationRequestRejection
    >
  >;
  /**
  ...
  capture(
    request: FieldMutationRequest,
    now: Timestamp,
  ): Result<QueueEntry, DomainError | FieldMutationRequestRejection>;
...
/**
 * Open the field session's data plane: issue the A9 grant, compose the
 ...
 * caller-supplied.
 */
export async function openFieldDataPlane(
  ...
  if (!grant.ok) {
    throw new TypeError(`field data plane wiring error (grant): ${grant.error.message}`);
  }
  ...
  if (!subscribed.ok) {
    throw new TypeError(`field data plane wiring error (subscribe): ${subscribed.error.message}`);
  }
 ...
 */
export const FIELD_SESSION_CORRELATION: CorrelationId = (() => {
  const parsed = parseCorrelationId('field-client-corr-0001');
  if (!parsed.ok) {
    throw new TypeError(
      `field client wiring error (session correlation): ${JSON.stringify(parsed.error)}`,
    );
  }
  return parsed.value;
})();
