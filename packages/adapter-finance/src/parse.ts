// Office adapter-finance — package-internal parse plumbing (OFF-024).
//
...
// combinator set (mirroring the adapters-sdk and adapter-construction
// internal conventions exactly: 'missing-field'/'unknown-field' codes, field
// paths nested under the parent field name) plus the combinators the
// provider-data shapes need beyond that set (bounded numbers, homogeneous
// arrays).
//
...
export const checkNumber = (value: unknown, rule: NumberRule, path: string): ParseResult<number> => {
  ...
  if (!Number.isInteger(value)) {
    return parseFail('invalid-value', path, `${rule.description} (integer)`, `number ${String(value)}`);
  }
...
/** Require a bounded integer numeric field of an object (no coercion, no NaN/Inf). */
...
/**
 * Parse a standalone array whose elements are validated by `parseValue`, in
 * order, with error paths nested like '<field>[3]'. Duplicate elements are
 * rejected (the closed-set discipline — e.g. resolution command keys).
 */
