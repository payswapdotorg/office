...
// OFF-033 package boundary self-gate — THE "suggestion-only recovery
// engine" acceptance: the revenue recovery engine is pure typed data +
// pure deterministic computation over the contracts model and the
// intelligence peers' outputs. This suite scans the package's own sources
// deterministically (filesystem reads only) and proves:
...
//   - the domain surface is @office/domain-contracts ONLY (the typed read
//     surface) — no other domain package (schedule/cost/field/organization/
//     projects/documents), no sync/client-sync/adapters/app-sdk/app-runtime/
//     marketplace/security/persistence/actions/events;
...
//     only PROPOSES — structurally no auto-assertion path);
 ...
 * build canonical contracts-domain records through the domain package's
 * OWN pure state transitions, exactly as a host would). The type-only
 * import disciplines below apply to the engine; the fixture modules are
 * the sanctioned host-side builders and are never re-exported.
 */
...
const FORBIDDEN_PACKAGES = [
  // Domain packages OTHER than domain/contracts (the dependency rule: the
  // recovery engine reads the canonical contracts/change model only).
  /^@office\/domain(?!-contracts|-kernel)(\/|$)/,
...
describe('recovery package boundary — the suggestion-only engine (OFF-033)', () => {
  it('is @office/intelligence-revenue 0.1.0, private, ESM, side-effect free', () => {
    const pkg = readPackageJson();
    expect(pkg['name']).toBe('@office/intelligence-revenue');
  ...
  it('consumes @office/domain-contracts TYPE-ONLY (the typed read surface, never its transitions)', () => {
    const violations: string[] = [];
    for (const file of engineFiles()) {
      const text = stripComments(readFileSync(join(srcDir, file), 'utf8'));
      for (const match of text.matchAll(
        /import\s+([^;]+?)\s*from\s+['"]@office\/domain-contracts['"]/g,
      )) {
        const clause = match[1] ?? '';
        if (!clause.trim().startsWith('type ')) {
          violations.push(`${file}: ${match[0]}`);
  ...
  it('the generic fixture vocabulary discipline (no real-world identities in the engine)', () => {
    ...
    // recovery kinds, the entity kinds of the contracts domain, the
    // command-name constants, and the engine identity.
    const text =
      stripComments(readFileSync(join(srcDir, 'vocabulary.ts'), 'utf8')) +
      stripComments(readFileSync(join(srcDir, 'candidates.ts'), 'utf8')) +
      stripComments(readFileSync(join(srcDir, 'model.ts'), 'utf8')) +
      stripComments(readFileSync(join(srcDir, 'detection.ts'), 'utf8'));
    expect(text).toContain("'constructive-change'");
    expect(text).toContain("'entitlement-rebalance'");
    expect(text).toContain("'delay-impact'");
    expect(text).toContain('intelligence-revenue');
    expect(text).toContain('recovery-candidate-detection');
  });

  it('structurally constructs NO command and executes NOTHING (the no-auto-assertion proof)', () => {
